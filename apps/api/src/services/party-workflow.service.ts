...
type CanonicalPartyCreateInput = Omit<PartyCreateInput, "photoConsent"> & {
  photoConsent: ReturnType<typeof normalizePhotoConsent>;
};

type PartyBookingDto = {
  id: string;
...
const PARTY_TRANSITIONS: Partial<
...
function melbourneLocalInstant(date: string, startTime: string): Date {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    const observedMinute =
      parseCalendarDate(observed.date).ordinal * 1_440 + observed.minuteOfDay;
...
export function decodePartyOperationNote(
  value: string | null,
  ...
  try {
    ...
    if (
      ...
      parsed === null ||
      !("partyWorkflow" in parsed) ||
      parsed.partyWorkflow !== 1
    )
      return null;
    if (
      "note" in parsed &&
    ...
    return {
      note:
        "note" in parsed && typeof parsed.note === "string"
          ? parsed.note
          : null,
    };
  } catch {
...
function time(value: number): string {
  if (value < 0 || value >= 24 * 60)
    throw new AppError(
      400,
      "VALIDATION_ERROR",
      "party setup and cleanup must remain on the operational date",
    );
...
function assertPartyInput(input: PartyCreateInput): void {
  ...
  if (
    ...
    input.participantCount > 8 ||
    !Number.isInteger(input.parentCount) ||
    input.parentCount < 1 ||
    input.parentCount > 2
  ) {
    throw new AppError(
      ...
      "Party guests must be 4–8 and parents must be 1–2",
    );
  }
  if (
    !Number.isInteger(input.birthdayChildAge) ||
    input.birthdayChildAge < PARTY_MINIMUM_BIRTHDAY_AGE
  )
    throw new AppError(
      400,
      "PARTY_BIRTHDAY_AGE_INVALID",
      `Birthday child must be at least ${PARTY_MINIMUM_BIRTHDAY_AGE}`,
    );
  if (
    !input.birthdayChildName.trim() ||
    !Array.isArray(input.projectInterests) ||
    input.projectInterests.length === 0 ||
    input.projectInterests.some((value) => !value.trim())
  ) {
    throw new AppError(
      400,
      "VALIDATION_ERROR",
      "birthdayChildName and projectInterests are required",
    );
  }
}
...
export function createPartyWorkflowService(
  db: Db,
  dependencies?: {
  now?: () => Date;
  ...
  const customerActionTokenSecret =
    dependencies?.customerActionTokenSecret ??
  ...
  function acceptTimeToken(bookingId: string, operationId: string): string {
    if (
      ...
      Buffer.byteLength(customerActionTokenSecret) < 32
    ) {
      throw new AppError(
        503,
    ...
    return createHmac("sha256", customerActionTokenSecret)
      .update(`party-accept:${bookingId}:${operationId}`)
      .digest("base64url");
  }

  async function packageFor(
    booking: { partyPackageId: string | null },
    tx: Db,
  ) {
    const partyPackage = booking.partyPackageId
      ? await partiesRepo.findById(booking.partyPackageId, tx)
      : null;
    if (
      !partyPackage ||
      !partyPackage.guestDurationMinutes ||
      !partyPackage.setupMinutes ||
      !partyPackage.cleanupMinutes
    )
      throw new AppError(
        422,
        "PARTY_PACKAGE_INVALID",
        "Party package is not configured for live booking",
      );
    return partyPackage;
  }

  async function validateGuestWindow(
    date: string,
    startTime: string,
    ...
    if (
      schedule.closures.some(
        (closure) =>
          ...
          (startTime < closure.endTime && end > closure.startTime),
      )
    ) {
      throw new AppError(
        400,
        "STUDIO_CLOSED",
        "The party guest time overlaps a closure",
      );
    }
  }

  async function assertCanHold(
    bookingId: string,
    interval: {
      date: string;
      setupStart: string;
    ...
    if (
      await availabilityRepo.hasExclusivePartyOverlap(
        {
          date: interval.date,
          startTime: interval.setupStart,
          endTime: interval.cleanupEnd,
        },
        tx,
    ...
    if (
      await availabilityRepo.sumConfirmedAttendance(
        {
          date: interval.date,
          startTime: interval.setupStart,
          endTime: interval.cleanupEnd,
      ...
      throw new AppError(
        409,
        "CAPACITY_CONFLICT",
        "The requested interval conflicts with a confirmed booking",
      );
    }
  }
  ...
  async function recordTransition(
    ...
    if (prior) {
      if (
        ...
        prior.fromStatus !== input.expectedStatus ||
        prior.toStatus !== input.toStatus ||
        prior.actorUserId !== input.actorUserId ||
        prior.adminNote !== encodePartyOperation(input.operationPayload)
      )
        throw new AppError(
          409,
          "OPERATION_ID_CONFLICT",
          "The operation ID belongs to a different party action",
        );
    ...
    if (!booking || booking.requestKind !== "party")
      throw new AppError(404, "NOT_FOUND", "Party booking not found");
    if (booking.status !== input.expectedStatus)
      throw new AppError(
        409,
        "STATUS_CONFLICT",
        "The party booking changed. Refresh and try again.",
      );
  ...
  async function enqueuePartyLifecycle(
    input: {
    ...
    if (!input.booking.email) {
      throw new AppError(
        422,
        "PARTY_EMAIL_MISSING",
        "Party booking email is required",
      );
    }
    const messageLocale = bookingLocale(input.booking.locale);
    const terminalPartyUpdate =
      input.template === "party_rejected" ||
      input.template === "party_cancelled";
  ...
  async function enqueuePartyCreated(
    input: {
    booking: Awaited<ReturnType<typeof bookingsRepo.findById>> & {};
    partyPackage: Awaited<ReturnType<typeof partiesRepo.findById>> & {};
      request: CanonicalPartyCreateInput;
    },
    tx: Db,
  ): Promise<void> {
    if (!input.booking.email) {
  ...
  async function assertPartyReplay(
    booking: Awaited<ReturnType<typeof bookingsRepo.findByIdempotencyKey>> & {},
    input: CanonicalPartyCreateInput,
    packageId: string,
    tx: Db = db,
  ): Promise<void> {
    if (
      ...
      booking.partyPackageId !== packageId
    ) {
      throw new AppError(
        409,
    ...
    const same =
      Boolean(details) &&
      booking.name === input.name.trim() &&
      booking.phone === input.phone.trim() &&
      booking.email === input.email.trim().toLowerCase() &&
      booking.locale === input.locale &&
      booking.policyVersion === input.policyVersion &&
      Boolean(booking.policyAcceptedAt) &&
      booking.photoConsentDecision === input.photoConsent.decision &&
      booking.photoConsentSignerName === input.photoConsent.signerName &&
      booking.photoConsentVersion === input.photoConsent.version &&
      Boolean(booking.photoConsentRecordedAt) &&
      booking.interestedProject === input.projectInterests.join(", ") &&
      booking.message === (input.specialRequirements?.trim() || null) &&
      details!.birthdayChildName === input.birthdayChildName.trim() &&
      details!.birthdayChildAge === input.birthdayChildAge &&
      details!.participantCount === input.participantCount &&
      details!.parentCount === input.parentCount &&
      details!.desiredDate === input.desiredDate &&
      details!.desiredStartTime === input.desiredStartTime &&
      details!.byoCake === input.byoCake &&
      details!.byoDrinks === input.byoDrinks &&
      details!.byoFood === input.byoFood &&
      details!.byoSnacks === input.byoSnacks &&
      details!.cakeCuttingRequested === input.cakeCuttingRequested &&
  ...
  return {
    async createPartyRequest(
      ...
      const canonicalInput = {
        ...input,
        projectInterests: canonicalProjectInterests(input.projectInterests),
        photoConsent: normalizePhotoConsent(input.photoConsent),
      };
      const key = assertUuid(idempotencyKey, "Idempotency-Key");
      const packageId = assertUuid(
        canonicalInput.partyPackageId,
        "partyPackageId",
      );
      ...
      return db.transaction(async (tx) => {
        ...
        if (
          (partyPackage.guestDurationMinutes !== 90 &&
            partyPackage.guestDurationMinutes !== 150) ||
          (partyPackage.guestDurationMinutes === 90 &&
            partyPackage.venueFeeCents !== 9500) ||
          (partyPackage.guestDurationMinutes === 150 &&
            partyPackage.venueFeeCents !== 14500) ||
          !partyPackage.setupMinutes ||
          !partyPackage.cleanupMinutes ||
          partyPackage.minSpendPerPersonCents !== 4500
        ) {
          throw new AppError(
            422,
            "PARTY_PACKAGE_INVALID",
        ...
        const schedule = await scheduleRepo.resolveDay(
          canonicalInput.desiredDate,
        );
        if (schedule.isClosed || !schedule.opensAt || !schedule.closesAt)
          throw new AppError(
            400,
        ...
        validateBookingWindow(
          {
            date: canonicalInput.desiredDate,
            startTime: canonicalInput.desiredStartTime,
            durationMinutes: partyPackage.guestDurationMinutes,
          },
          getMelbourneClock(now()),
          { opensAt: schedule.opensAt, closesAt: schedule.closesAt },
        );
    ...
    async proposePartyTime(input: {
      ...
      if (
        !(input.paymentDeadline instanceof Date) ||
        Number.isNaN(input.paymentDeadline.getTime())
      )
        throw new AppError(
          400,
          "VALIDATION_ERROR",
          "paymentDeadline must be a valid date",
        );
      const operationPayload = {
        action: "propose",
        finalDate: input.finalDate,
        finalGuestStart: input.finalGuestStart,
        paymentDeadline: input.paymentDeadline.toISOString(),
      };
      return db.transaction(async (tx) => {
        ...
        if (prior) {
          if (
            prior.bookingId !== bookingId ||
            prior.fromStatus !== input.expectedStatus ||
            prior.actorUserId !== actorUserId ||
            prior.adminNote !== encodePartyOperation(operationPayload) ||
            (prior.toStatus !== "time_proposed" &&
              prior.toStatus !== "awaiting_in_store_payment")
          )
            throw new AppError(
              409,
              "OPERATION_ID_CONFLICT",
          ...
          if (prior.toStatus === "time_proposed") {
            ...
            if (!details?.paymentDeadline)
              throw new AppError(
                409,
                "STATUS_CONFLICT",
                "Party proposal is incomplete",
              );
            if (
              replay.status !== "time_proposed" ||
              details.paymentDeadline <= now()
            ) {
              return {
                id: replay.id,
                status: replay.status,
            ...
            return {
              ...
              createdAt: replay.createdAt,
              replayed: true,
              acceptTimeToken: acceptTimeToken(bookingId, operationId),
            };
          }
          return {
            id: replay.id,
        ...
        if (!details)
          throw new AppError(
            404,
            "NOT_FOUND",
            "Party booking details not found",
          );
        const partyPackage = await packageFor(booking, tx);
        const duration = partyPackage.guestDurationMinutes as 90 | 150;
        await validateGuestWindow(
          input.finalDate,
          input.finalGuestStart,
          duration,
          tx,
        );
        const interval = partyInterval(
          input.finalDate,
          input.finalGuestStart,
          duration,
          partyPackage.setupMinutes!,
        ...
        const sameDesired =
          details.desiredDate === input.finalDate &&
          details.desiredStartTime === input.finalGuestStart;
        const target = sameDesired
          ? "awaiting_in_store_payment"
          : ("time_proposed" as const);
        if (sameDesired) await assertCanHold(bookingId, interval, tx);
        const updated = await partyRepo.setProposal(
          {
            bookingId,
            expectedStatus: input.expectedStatus,
            date: input.finalDate,
            setupStart: interval.setupStart,
            guestStart: interval.guestStart,
            guestEnd: interval.guestEnd,
            cleanupEnd: interval.cleanupEnd,
        ...
        await tokensRepo.create(
          {
            ...
            scopes: ["accept_time"],
            expiresAt: input.paymentDeadline,
          },
          tx,
        );
        await enqueuePartyLifecycle(
    ...
    async acceptPartyTime(input: {
      ...
      return db.transaction(async (tx) => {
        ...
        const transition = await recordTransition(
          {
            ...
            operationId,
            actorUserId,
            actorKind: actorUserId ? "staff" : "customer",
            operationPayload,
          },
          tx,
        );
        ...
        if (
          !details?.finalDate ||
          !details.finalSetupStart ||
          !details.finalGuestStart ||
          !details.finalGuestEnd ||
          !details.finalCleanupEnd ||
          ...
          throw new AppError(
            409,
            "PARTY_HOLD_EXPIRED",
            "The proposed party time is no longer available",
          );
        await assertCanHold(
          bookingId,
          {
        ...
        const updated = await partyRepo.setStatus(
          bookingId,
          "time_proposed",
          "awaiting_in_store_payment",
          tx,
    ...
    async recordPartyPayment(input: {
      ...
      return db.transaction(async (tx) => {
        ...
        const details = await partyRepo.findDetails(bookingId, tx);
        if (!details || details.venueFeeCents !== input.amountCents)
          throw new AppError(
            400,
            "PARTY_PAYMENT_AMOUNT_INVALID",
            "The in-store payment must equal the venue fee",
          );
        if (details.paymentDeadline && details.paymentDeadline <= now())
          throw new AppError(
            409,
            "PARTY_HOLD_EXPIRED",
            "The party hold has expired",
          );
        const updated = await partyRepo.setStatus(
          bookingId,
          "awaiting_in_store_payment",
        ...
        await partyRepo.recordPayment(
          bookingId,
          input.paidAt,
          input.amountCents,
          tx,
        );
        await partyRepo.addCharge(
          {
            bookingId,
            type: "venue_fee",
            amountCents: input.amountCents,
        ...
        if (
          ...
          throw new AppError(
            409,
            "PARTY_PROPOSAL_INCOMPLETE",
            "Party final time is required",
          );
        }
        await enqueuePartyLifecycle(
          {
    ...
    async transitionPartyStatus(input: {
      bookingId: string;
      expectedStatus: BookingStatus;
      toStatus: BookingStatus;
      operationId: string;
      ...
      const operationId = assertUuid(input.operationId, "operationId");
      const actorUserId = assertUuid(input.actorUserId, "actorUserId");
      if (!PARTY_TRANSITIONS[input.expectedStatus]?.includes(input.toStatus))
        throw new AppError(
          400,
          "PARTY_DEDICATED_ACTION_REQUIRED",
          "This party transition requires its dedicated operational action",
        );
      return db.transaction(async (tx) => {
        const operationPayload = {
          action: "transition",
        ...
        if (input.toStatus === "rejected" || input.toStatus === "cancelled") {
          ...
          const endTime =
            ...
            time(
              minutes(startTime) +
                (await packageFor(updated, tx)).guestDurationMinutes!,
          );
          await enqueuePartyLifecycle(
            {
            booking: updated,
    ...
    async acceptPartyTimeByToken(rawToken: string): Promise<PartyBookingDto> {
      ...
      return db.transaction(async (tx) => {
        const token = await tokensRepo.findActiveByDigest(
          createHash("sha256").update(rawToken).digest("hex"),
          now(),
          tx,
        );
        if (!token || !token.scopes.includes("accept_time"))
          throw invalidLink();
    ...
    async expirePartyHold(input: {
      ...
      const actorUserId =
        ...
        ? null
        : assertUuid(input.actorUserId, "actorUserId");
      if (input.expectedStatus !== "awaiting_in_store_payment")
        throw new AppError(
          400,
          "VALIDATION_ERROR",
          "expectedStatus must be awaiting_in_store_payment",
        );
      ...
      return db.transaction(async (tx) => {
        ...
        const transition = await recordTransition(
          {
            ...
            operationId,
            actorUserId,
            actorKind: actorUserId ? "staff" : "system",
            operationPayload,
          },
          tx,
        );
        ...
        if (booking.status !== expectedStatus)
          throw new AppError(
            409,
            "STATUS_CONFLICT",
            "Party hold is not awaiting payment",
          );
        const details = await partyRepo.findDetails(bookingId, tx);
        if (!details?.paymentDeadline || details.paymentDeadline > current)
          throw new AppError(
            409,
            "PARTY_HOLD_NOT_EXPIRED",
            "Party hold has not expired",
          );
        const updated = await partyRepo.setStatus(
          bookingId,
          expectedStatus,
    ...
    async recordPartyCharge(input: {
      ...
      const actorUserId = assertUuid(input.actorUserId, "actorUserId");
      if (
        input.type !== "cake_cutting" &&
        input.type !== "cleaning" &&
        input.type !== "overtime"
      ) {
        throw new AppError(
          400,
          "PARTY_CHARGE_TYPE_INVALID",
          "Party charge type is invalid",
        );
      }
      const validAmount =
        input.type === "cake_cutting"
          ? input.amountCents === 1500
          : input.amountCents >= 1500 && input.amountCents <= 3500;
      if (!validAmount)
        throw new AppError(
          400,
          "PARTY_CHARGE_AMOUNT_INVALID",
          "Party charge amount is invalid",
        );
      return db.transaction(async (tx) => {
        const operationPayload = {
          action: "charge",
    ...
    async recordPartyRefund(input: {
      ...
      const actorUserId = assertUuid(input.actorUserId, "actorUserId");
      if (
        !(input.refundedAt instanceof Date) ||
        Number.isNaN(input.refundedAt.getTime())
      )
        throw new AppError(
          400,
          "VALIDATION_ERROR",
          "refundedAt must be a valid date",
        );
      return db.transaction(async (tx) => {
        const operationPayload = {
          action: "refund",
        ...
        if (
          !details ||
          !venueFee ||
          venueFee.amountCents !== details.venueFeeCents
        )
          throw new AppError(
            409,
            "PARTY_REFUND_PAYMENT_MISSING",
            "A recorded venue fee is required before refunding",
          );
        if (!details.finalDate || !details.finalGuestStart)
          throw new AppError(
            409,
            "PARTY_REFUND_INELIGIBLE",
            "Party final time is required before refunding",
          );
        const cancellation = await eventsRepo.findLatestWithStatus(
          bookingId,
        ...
        if (!cancellation)
          throw new AppError(
            409,
            "PARTY_REFUND_INELIGIBLE",
            "A customer cancellation request is required before refunding",
          );
        const localStart = melbourneLocalInstant(
          details.finalDate,
          details.finalGuestStart,
        );
        if (
          localStart.getTime() - cancellation.createdAt.getTime() <
          48 * 60 * 60 * 1000
        )
          throw new AppError(
            409,
            "PARTY_REFUND_INELIGIBLE",
            "Party venue fees are refundable only 48 hours before the guest start",
          );
        const updated = await partyRepo.setStatus(
          bookingId,
          "cancelled",
        ...
        await partyRepo.addCharge(
          {
            bookingId,
            type: "refund",
            amountCents: details.venueFeeCents,
            recordedByUserId: actorUserId,
          },
          tx,
        );
...
export type PartyWorkflowService = ReturnType<
  typeof createPartyWorkflowService
>;
