...
const chinese: Record<string, string> = {
  ...
  paymentBody:
    "场地费同时作为订金，需在派对日期前另行到店支付。确认后由店员告知付款期限。网站不提供线上付款。",
  refund:
...
describe("PartiesPage", () => {
  ...
  it("keeps the capability-off page useful, truthful, and non-submittable", async () => {
    ...
    expect(container.textContent).toContain(
      "separate visit before the party date"
    )
    expect(container.textContent).toContain(
      "staff will tell you the payment deadline"
    )
    ...
    expect(
      container.querySelector('a[href="mailto:congdongdong03@gmail.com"]')
    ).not.toBeNull()
  })
  ...
  it("uses a verified party image without changing the request gate", async () => {
    ...
    expect(
      container.querySelector('img[alt="YezYY party table"]')
    ).not.toBeNull()
    expect(container.querySelector("form")).toBeNull()
  })
